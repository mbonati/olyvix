...
var VIZ_SIZE = 300;
var VIZ_WIDTH = VIZ_SIZE;
var VIZ_HEIGHT = VIZ_SIZE;

var loading_timer = null;
var dataLoading = false;
var returning_back = false;
var w = VIZ_WIDTH, /*innerWidth,*/
    h = VIZ_HEIGHT, /*innerHeight,*/
    w2 = w/2,
    h2 = h/2,
...
// -- settings
var settings = {
  MAIN_BALL_RADIO: VIZ_WIDTH/3,
  MAX_LINE_SIZE: VIZ_HEIGHT/2  ,
};
...
function resizeRayViz(){
      w = jQuery("#rayViz").width();
      h = jQuery("#rayViz").height();
      svg.attr("width", w);
      svg.attr("height", h);
...
function createViz(){
    ...
    lines = svg.append('svg:g')
      .attr("transform", "translate(" + w/2 + "," +  0/2 +" )");
    ...
    var strokeWidth = (VIZ_WIDTH*1.3) / allCountries.length;
...
function loadCrossFilter(){
  d3.csv(BASE_DATA_URL + "vwMedalsByCountry.csv", function(medalsView) {
      
      // feed it through crossfilter
      var medalsCF = crossfilter(medalsView);
      var allMedals = medalsCF.groupAll();

      // dimensioni edizioni
      var gameIdDim = medalsCF.dimension(function(d){ return d.gamesId;} );
      var gamesId = gameIdDim.group();

      // dimensioni countryCode
      var countryCodeDim = medalsCF.dimension(function(d){ return d.countryCode;} );
      var countryCodes = countryCodeDim.group();

      // define a dimension
      var medalsByEditionYear = medalsCF.dimension(function(d) { return d.year; });
      // map/reduce to group sum
      var medalsByEditionYearGroup = medalsByEditionYear.group().reduceSum(function(d) { return d.total; });
      var medalsExByEditionYearGroup = medalsByEditionYear.group().reduce(
          ...
          function(p, v) {
              ...
              p.totalBronze += parseInt(v.bronze);
              p.participants++;
              p.year = v.year;
              p.hostCity = v.hostCity;
              return p;
          },
          //Remove function
          function(p, v) {
